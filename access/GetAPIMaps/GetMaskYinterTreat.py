import numpy as np
import pandas as pd
import pydicom
from PIL import Image

from src.APIMaps.GetAPI import getAPI
from src.Separation import Separate
from utils.helperfunction import sort_files_numerically


def GetInlets(path):
    pre, _, _ = Separate.SepInlet(path)
    data = {}

    for key, file_list in pre.items():
        if len(file_list) > 1:
            file_list = sort_files_numerically(file_list)

        for idx, file_path in enumerate(file_list):
            try:
                with Image.open(file_path).convert("L") as img:
                    arr = np.array(img, dtype=np.float32) > 0
                    name = f"{key}"
                    if len(file_list) > 1:
                        name += f"_View{idx + 1}"
                    data[name] = arr
            except Exception as e:
                print(f"Failed to load {file_path}: {e}")
    return data


def GetImages(path):
    pre, _, _ = Separate.SepImages(path)
    data = {}
    path_dict = {}
    for key, file_list in pre.items():
        if len(file_list) > 1:
            file_list = sort_files_numerically(file_list)
        for idx, file_path in enumerate(file_list):
            try:
                ds = pydicom.dcmread(file_path)
                img_array = ds.pixel_array.astype(np.float32)
                name = f"{key}"
                if len(file_list) > 1:
                    name += f"_View{idx + 1}"
                data[name] = img_array
                path_dict[name] = ds
            except Exception as e:
                print(f"Failed to read {file_path}: {e}")
    return data, path_dict

def GetMasks(path):
    pre, _, _ = Separate.SepMasks(path)
    data = {}

    for key, file_list in pre.items():
        if len(file_list) > 1:
            file_list = sort_files_numerically(file_list)

        for idx, file_path in enumerate(file_list):
            try:
                with Image.open(file_path).convert("L") as img:
                    arr = np.array(img, dtype=np.float32) > 0
                    name = f"{key}"
                    if len(file_list) > 1:
                        name += f"_View{idx + 1}"
                    data[name] = arr
            except Exception as e:
                print(f"Failed to load {file_path}: {e}")
    return data

if __name__ == "__main__":
    img_path = r"Z:\Users\Artin\coiled\raw_file"
    inlet_path = r"Z:\Users\Artin\coiled\inlets"
    mask_path = r"Z:\Users\Artin\coiled\aneurysms"

    img_data, path_dict = GetImages(img_path)
    inlet_data = GetInlets(inlet_path)
    mask_data = GetMasks(mask_path)

    data_dict = {}
    for case, data in img_data.items():
        if case in inlet_data and case in mask_data:
            api_obj = getAPI(
                dsa=data,
                inlet=inlet_data[case],
                dsa_temp=path_dict[case],
                mask=mask_data[case],
                frac=0.1,
                show_mask_stats=False,
                show_okelly_scale=False,
                show_mask_tdconly=True
            )
            y_inter = api_obj.get_avgtdc_mask()
            data_dict[case] = y_inter
    df = pd.DataFrame.from_dict(data_dict, orient='index')
    df.to_csv("TdcMaskPost.csv", index=True)





